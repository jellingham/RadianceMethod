...
def plot_ec_all(results_dir, exp_name, x_min, x_max, y_min, y_max):
    for channel in [0,1,2]:
        ...
        plot_ec(data_path, exp_name, channel, x_min, x_max, y_min, y_max)

def plot_ec(data_path, exp_name, channel, x_min, x_max, y_min, y_max):
    data = pd.read_csv(data_path, skiprows=3, parse_dates=['Timedelta'])
    ...
    plt.savefig(f'{exp_name}_plot_ec_channel_{channel}.pdf')
    plt.close()
